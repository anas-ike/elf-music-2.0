...
description: Learn how to invite and authorize Mewwme to your Discord server to start enjoying its features.
...
Click the `Select a server` button. A pop-up will appear allowing you to choose where you’d like to add Mewwme.

### Step 2

After selecting the server you want to add Mewwme to, click the `Continue` button at the bottom right of the menu.
...
>
  ...
  from Mewwme, it will not work properly! You will still be able to use basic
...
>
  ...
  either `Unlisted` or `Public`. If it is set to `Private`, Mewwme won’t be able
...
## Mewwme is packed with features to ensure **you** have the most fun with your friends.

### 🔎 Play and Search

Mewwme can play songs in your voice channel from sources like **Spotify**, **Soundcloud**, **Apple Music**, and _many_ more! Check out the [Frequently Asked Questions](https://meww.me/blog/frequently-asked-questions) for the full list of supported sources. It can also play playlists, which are added to a queue system. We provide the top 10 search 🔎 results from Spotify when you use the `searchsong` command.

## 📃 Lyrics

When Mewwme is playing a song, you can get the lyrics from [Genius](https://genius.com) for the most accurate song lyrics. Host your own karaoke night with lyrics readily available! Use the `lyrics` command followed by a song name to manually search for lyrics.

## 🎙️ Advanced Features

### Turntable Controls

Control the turntable with commands such as `me seek`, `me back`, `meskip`, and more while listening to Mewwme.

### Effects

Enjoy special sound effects for **free**! Modify and adjust playback to enhance your experience with Mewwme.
...
Ready to start listening with Mewwme? It’s super easy, so let’s dive in!
